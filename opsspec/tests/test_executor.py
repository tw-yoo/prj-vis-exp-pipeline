...
class ExecutorTest(unittest.TestCase):
    ...
    def test_chained_filter_intersection(self) -> None:
        # Two filters on the same field chained via meta.inputs: the second filter
        # runs on the first's row result, so the surviving targets are the
        # intersection of the two include lists (replacement for the removed setOp).
        ...
        targets = sorted({item.target for item in results["ops2"]})
        self.assertEqual(targets, ["2017/18"])
