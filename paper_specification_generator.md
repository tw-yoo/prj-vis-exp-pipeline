...
자연어만으로는 어떤 필드가 차트에 존재하는지, 어떤 값이 가능한지 확신할 수 없습니다.  
...
이 정보는 이후 모든 단계가 **모호한 자연어를 실제 데이터에 맞게 정합화(grounding)**하는 기준이 됩니다.
...
- 노드 ID나 메타 정보는 이 단계에서 만들지 않는다(결정론적으로 부여된다).
...
그래프가 커질수록 meta.inputs 순서/중복 문제가 발생할 수 있습니다.  
따라서 마지막에 다음을 정리합니다.
