...
- Phrase mapping (non-draw meaning behind common visual phrases):
  ...
  - "total of" / "sum of all" / "sum of X" / "add up all" (dataset row aggregation, NOT scalar arithmetic) implies a "sum" task. `sum` is BAR CHARTS ONLY (simple/stacked); group may be a single series or list of series. For scalar addition of two prior computed values, use "add" instead. Check chart_context.mark: on a line chart (mark = "line", simple/multiple) the contract rejects `sum`, so do NOT emit a sum task there — if the explanation totals two specific points, emit an "add" task over those two point values instead.
